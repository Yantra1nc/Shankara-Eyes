 ...
 */
 define(['N/currentRecord', 'N/search', 'N/record', 'N/log', 'N/url', 'N/ui/serverWidget', 'N/email', 'N/runtime', 'N/file', 'N/render'], function(currentRecord, search, record, log, url, serverWidget, email, runtime, file, render) {
		...
		//if (context.type == context.UserEventType.CREATE || context.type == context.UserEventType.EDIT)
		{
			...
			*/
			...
			if(_logValidation(vendor_email)) {
				log.debug("checking for email on vendor");
				if(record_Type == 'purchaseorder') {
					...
					var check = record_load.getValue({
						fieldId: 'custbody_seh_email_sent_'
					});
						...
						if(approval_status_po == "2") {
							if(check == false) {
								...
								if(_logValidation(ccArr)) {
									...
									var check_set = record_load.setValue({
										fieldId: 'custbody_seh_email_sent_',
										value: true,
				...
				if(record_Type == 'vendorpayment') {
					log.debug("entered vendor payment");
					/*var transactionFile = render.transaction({
					entityId: recordId,
					printMode: render.PrintMode.PDF,
					//inCustLocale: true
